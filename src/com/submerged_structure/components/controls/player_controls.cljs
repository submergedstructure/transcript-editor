...
(defn time-to-skip-to-in-pause-between-segments [pause-start pause-end]
  (+
   (max (+ (if pause-start pause-start 0) 0.2)
        (- pause-end 0.7))
   0.001)) ;; add a millisecond as after .setTime .currentTime is off by a fraction of a millisecond.
...
(defsc PlayerControls [this {:transcript/keys [id]
  ...
  {:ident :transcript/id
   :query [:transcript/id
           ...
           :ui-player-controls/prev-prev-segment-end
           :ui-player-controls/prev-segment-end
           :ui-player-controls/prev-segment-start
           :ui-player-controls/current-segment-start
           :ui-player-controls/current-segment-end
